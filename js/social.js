...
async function aceitarPedido(meuUid, fromUid) {
    try {
        // 1. Criar a amizade para AMBOS os lados usando UPSERT
        // Isso garante que o registro pendente mude para 'accepted' 
        // e cria a via de volta (sua para o amigo)
        const { error } = await supabase.from('friends').upsert([
            { uid: fromUid, friend_uid: meuUid, status: 'accepted' },
            { uid: meuUid, friend_uid: fromUid, status: 'accepted' }
        ], { onConflict: 'uid,friend_uid' });

        if (error) throw error;

        // 2. Marcar a notificação como lida para ela sumir da sidebar
        await supabase.from('notificacoes')
            .update({ lida: true })
            .eq('to_uid', meuUid)
            .eq('from_uid', fromUid)
            .eq('tipo', 'pedido_amizade');

        console.log("Amizade aceita!");
        
        // 3. Atualizar a interface
        if (typeof recarregarSidebar === 'function') {
            await recarregarSidebar(meuUid);
        }

    } catch (err) {
        console.error("Erro ao aceitar pedido:", err.message);
        alert("Erro ao aceitar amizade. Verifique sua conexão.");
    }
}

async function recusarPedido(meuUid, fromUid) {
    try {
        // 1. Deleta o pedido pendente
...
// IMPORTANTE: Como você usa type="module", as funções precisam ser expostas globalmente
// para que os botões onclick do seu HTML (index.html) funcionem.
